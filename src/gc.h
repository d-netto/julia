...
#define GC_PAGE_OFFSET (2 * JL_HEAP_ALIGNMENT - (sizeof(jl_taggedvalue_t) % JL_HEAP_ALIGNMENT))
...
#define GC_ALLOC_MAP_LG2 ((sizeof(void*) * 8 - GC_PAGE_LG2) / 2)

typedef struct {
    uint8_t alloc[1ull << GC_ALLOC_MAP_LG2];
} jl_gc_alloc_map_inner_t;

typedef struct {
    jl_mutex_t lock;
    jl_gc_alloc_map_inner_t *alloc[1ull << GC_ALLOC_MAP_LG2];
} jl_gc_alloc_map_t;

extern jl_gc_alloc_map_t alloc_map;

STATIC_INLINE size_t gc_alloc_map_inner_idx(void *_data) JL_NOTSAFEPOINT
{
    return (size_t)(((uintptr_t)(_data) << GC_ALLOC_MAP_LG2) >> (GC_ALLOC_MAP_LG2 + GC_PAGE_LG2));
}

STATIC_INLINE size_t gc_alloc_map_idx(void *_data) JL_NOTSAFEPOINT
{
    return (size_t)((uintptr_t)(_data) >> (GC_ALLOC_MAP_LG2 + GC_PAGE_LG2));
}

STATIC_INLINE uint8_t gc_alloc_map_is_set(char *_data) JL_NOTSAFEPOINT
{
    jl_gc_alloc_map_inner_t *inner = alloc_map.alloc[gc_alloc_map_idx(_data)];
    if (inner != NULL) {
        return inner->alloc[gc_alloc_map_inner_idx(_data)];
    }
    return 0;
}
...
STATIC_INLINE void gc_alloc_map_maybe_create(char *_data) JL_NOTSAFEPOINT
{
    jl_gc_alloc_map_inner_t *inner = alloc_map.alloc[gc_alloc_map_idx(_data)];
    if (inner == NULL) {
        inner = (jl_gc_alloc_map_inner_t*)calloc_s(sizeof(jl_gc_alloc_map_inner_t));
        alloc_map.alloc[gc_alloc_map_idx(_data)] = inner;
    }
}
...
STATIC_INLINE void gc_alloc_map_set(char *_data, uint8_t v) JL_NOTSAFEPOINT
{
    jl_gc_alloc_map_inner_t *inner = alloc_map.alloc[gc_alloc_map_idx(_data)];
    assert(inner != NULL);
    inner->alloc[gc_alloc_map_inner_idx(_data)] = v;
}
// Page layout:
//  Newpage freelist: sizeof(void*)
//  Metadata pointer: sizeof(jl_gc_pagemeta_t*)
//  Padding: GC_PAGE_OFFSET - sizeof(void*) - sizeof(jl_gc_pagemeta_t*)
//  Blocks: osize * n
...
{
    return *(jl_gc_pagemeta_t**)(gc_page_data(_data) + sizeof(void*));
}

STATIC_INLINE void set_page_metadata(jl_gc_pagemeta_t *pg) JL_NOTSAFEPOINT
{
    *(jl_gc_pagemeta_t**)(pg->data + sizeof(void*)) = pg;
}
