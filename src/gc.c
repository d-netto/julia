...
static void NOINLINE gc_mark_stack_resize(jl_gc_mark_sp_t *sp) JL_NOTSAFEPOINT
{
    void **old_pc = sp->pc_start;
    jl_gc_mark_data_t *old_data = sp->data_start;

    size_t stack_size = (sp->pc_end - old_pc);

    sp->pc_start = (void **)realloc_s(old_pc, 2 * stack_size * sizeof(void *));
    sp->data_start = (jl_gc_mark_data_t *)realloc_s(
        old_data, 2 * stack_size * sizeof(jl_gc_mark_data_t));

    sp->pc = sp->pc_start + stack_size;
    sp->data = (jl_gc_mark_data_t *)(((char *)sp->data - (char *)old_data) +
                                     (char *)sp->data_start);

    sp->pc_end = sp->pc_start + 2 * stack_size;
}
...
STATIC_INLINE void gc_mark_stack_push(jl_gc_mark_sp_t *sp, void *pc, void *data,
                                      size_t data_size, int inc) JL_NOTSAFEPOINT
{
    assert(data_size <= sizeof(jl_gc_mark_data_t));
    if (__unlikely(sp->pc == sp->pc_end))
        gc_mark_stack_resize(sp);
    *sp->pc = pc;
                           ...
                           arraylist_t *list, size_t start)
{
    ...
    gc_mark_stack_push(sp, gc_mark_label_addrs[GC_MARK_L_finlist], &markdata,
                       sizeof(markdata), 1);
}
                                          ...
                                          jl_value_t *obj)
{
    ...
    gc_mark_stack_push(sp, gc_mark_label_addrs[GC_MARK_L_scan_only], &data, sizeof(data),
                       1);
}
                                    ...
                                    void *_obj) JL_NOTSAFEPOINT
{
    ...
    if (!gc_try_setmark(obj, &nptr, &tag, &bits))
        return (int)nptr;
    gc_mark_marked_obj_t data = {obj, tag, bits};
    gc_mark_stack_push(sp, gc_mark_label_addrs[GC_MARK_L_marked_obj], &data, sizeof(data),
                       1);
    return (int)nptr;
}
                                            ...
                                            jl_value_t **objs, size_t nobjs)
{
    ...
    gc_mark_stack_push(&ptls->gc_mark_sp, gc_mark_label_addrs[GC_MARK_L_objarray], &data,
                       sizeof(data), 1);
}
...
STATIC_INLINE int gc_mark_scan_array16(jl_ptls_t ptls, jl_gc_mark_sp_t *sp,
                                       gc_mark_array16_t *ary16, jl_value_t **begin,
                                       jl_value_t **end, uint16_t *elem_begin,
                                       uint16_t *elem_end, jl_value_t **pnew_obj,
                                       uintptr_t *ptag, uint8_t *pbits)
{
...
array16_loaded:
    if (gc_mark_scan_array16(ptls, sp, ary16, objary_begin, objary_end, obj16_begin,
                             obj16_end, &new_obj, &tag, &bits))
        goto mark;
    goto pop;
...
obj8:
    ...
    obj8_end = obj8->end;
obj8_loaded:
    if (gc_mark_scan_obj8(ptls, sp, obj8, obj8_parent, obj8_begin, obj8_end, &new_obj, &tag,
                          &bits))
        goto mark;
    goto pop;
...
module_binding : {
        ...
        if (value) {
            ...
            if (gc_try_setmark(value, &binding->nptr, &tag, &bits)) {
                ...
                if (gc_try_setmark(globalref, &binding->nptr, &gr_tag, &gr_bits)) {
                    ...
                    gc_mark_stack_push(sp, gc_mark_laddr(marked_obj), &data, sizeof(data),
                                       1);
                }
                goto mark;
            }
    ...
    if (nusings) {
        ...
        gc_mark_objarray_t data = {(jl_value_t *)m, objary_begin, objary_end, 1,
                                   binding->nptr};
        gc_mark_stack_push(sp, gc_mark_laddr(objarray), &data, sizeof(data), 0);
        if (!scanparent) {
...
mark : {
    ...
    if (vt == jl_simplevector_type) {
        ...
        gc_mark_stack_push(sp, gc_mark_laddr(objarray), &markdata, sizeof(markdata), 0);
        objary = (gc_mark_objarray_t *)sp->data;
        goto objarray_loaded;
    }
        ...
        if (flags.ptrarray) {
            ...
            if (npointers == 1) { // TODO: detect anytime time stride is uniform?
                ...
                gc_mark_objarray_t markdata = {new_obj, objary_begin, objary_end, elsize,
                                               nptr};
                gc_mark_stack_push(sp, gc_mark_laddr(objarray), &markdata, sizeof(markdata),
                                   0);
                objary = (gc_mark_objarray_t *)sp->data;
                goto objarray_loaded;
            }
                ...
                gc_mark_stack_push(sp, gc_mark_laddr(array8), &markdata, sizeof(markdata),
                                   0);
                ...
                gc_mark_stack_push(sp, gc_mark_laddr(array16), &markdata, sizeof(markdata),
                                   0);
        ...
        gc_mark_stack_push(sp, gc_mark_laddr(module_binding), &markdata, sizeof(markdata),
                           0);
        ...
        if (s) {
            ...
            gc_mark_stack_push(sp, gc_mark_laddr(stack), &stackdata, sizeof(stackdata), 1);
        }
        if (ta->excstack) {
            ...
            gc_mark_stack_push(sp, gc_mark_laddr(excstack), &stackdata, sizeof(stackdata),
                               1);
        }
        ...
        gc_mark_stack_push(sp, gc_mark_laddr(obj8), &markdata, sizeof(markdata), 0);
        ...
        if (layout->fielddesc_type == 0) {
            ...
            gc_mark_stack_push(sp, gc_mark_laddr(obj16), &markdata, sizeof(markdata), 0);
            ...
            gc_mark_stack_push(sp, gc_mark_laddr(obj32), &markdata, sizeof(markdata), 0);
            sp->data = (jl_gc_mark_data_t *)(((char *)sp->data) + sizeof(markdata));
            goto obj32;
        }
        else {
...
void jl_init_thread_heap(jl_ptls_t ptls)
{
    ...
    sp->pc_start = sp->pc = (void **)malloc_s(init_size * sizeof(void *));
    sp->pc_end = sp->pc_start + init_size;
    sp->data_start = sp->data =
